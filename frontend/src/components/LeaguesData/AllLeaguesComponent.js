    ...
    if (params) {
      ...
      if (params["country-name"]) {
        nvs.push({
          ...
          url: `/${stringToSlug(params["country-name"])}/${stringToSlug(
            params["sport-name"]
          )}`,
        });
      }
  ...
  const meta = {
    title: params
      ? `${
          ...
          " " +
          (doFormatText(countryName) == undefined
            ? ""
            : doFormatText(countryName)) +
          " "
    ...
    description: params
      ? `Compare best betting lines and betting odds for the ${
          doFormatText(countryName) + " " + sportsName == "soccer"
            ? "Football"
            : sportsName + ", " + sportsName + " "
        } leagues and tournaments. Explore odds arbitrage, results and find the best betting bookmakers.`
      : "",
        ...
        keywords: params
          ? `${
              ...
              (doFormatText(countryName) == undefined
                ? ""
                : doFormatText(countryName) + " ")
            }leagues, tournaments, odds comparison, dropping odds, matched betting, compare sport odds, predictions, tips, results, betting lines`
          : "",
      },
    },
  };
    ...
    <>
      <div className={`${droppingStyle.droppingOddsBg} mt-3`}>
        ...
        <AboutDroppingOdds
          disciption={true}
          title={`${doFormatText(
            params["sport-name"] == "soccer" ? "football" : params["sport-name"]
                  ...
                  return (
                    <Col md={6} key={eventIndex} className="pb-1">
                      <div
                        key={eventIndex}
                        className={
                          topEventsIndices.indexOf(eventIndex) !== -1
                            ? myCouponStyle.SetPlace
                            : myCouponStyle.bgTopEvents
                        }
                      >
                        <NavLink
                          to={`/matches/${event.sport}/${event.country}/${event.name}`}
                        >
